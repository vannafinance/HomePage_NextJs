...
export default function JoinWaitlist() {
    ...
    const authenticateUser = async () => {
        try {
            const authen = await fetch("api/auth");
            const data = await authen.json();
            if (data) {
                setUser(data);
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (!response.ok || !eligibility.isCompleted) {
                ...
                setJoinwaitlistMessage(`Quest not completed for ${formData.evm_address}. Complete the Galxe quest first to join the waitlist.`);
                return;
            }
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ...
            setJoinwaitlistpop(true);
            setJoinwaitlistMessage(`Error checking quest completion: ${err.message}. Please try again.`);
            return;
        }
                        ...
                        {popUpJoinwaitlist && (
                            <div className="fixed inset-0 flex items-center justify-center z-50">
                                ...
                                <div className="relative bg-white rounded-xl p-6 max-w-md w-full text-center shadow-lg z-60 max-h-[80vh] overflow-y-auto">
                                    ...
                                    <h3 className="text-lg font-semibold text-red-600 mb-2">Galxe Verification Failed</h3>
                                    <p className="text-gray-700 mb-6 text-sm leading-relaxed">
                                        {popUpJoinwaitlistmessage || "Your EVM address couldn't be verified on Galxe. To avoid disqualification from quests and the waitlist race, please complete verification first. Once verified, return here to join securely!"}
                                    </p>
                                    ...
                                    <p className="mt-4 text-xs text-gray-500">
                                        Need help? <a href="https://docs.galxe.com/" target="_blank" rel="noopener noreferrer" className="underline hover:text-blue-600">Galxe Docs</a>
                                    </p>
                                </div>
                            </div>
                        )}
